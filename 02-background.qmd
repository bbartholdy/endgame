...
a few hundred different species living within the mouth a single person.
...
Before this, researchers believed that you could isolate the organism of
...
One such baterial community is a biofilm. Biofilms consist of large multi-species
...
and immune responses from the host that would normally affect their
...
different species of bacteria [@flemmingBiofilmsEmergent2016]. Biofilms are
quite persistent structures, and very few surfaces exist that can inhibit
bacterial colonisation and biofilm formation <!-- rephrase --> [@rennerPhysicochemicalRegulation2011].
...
to attach, which is the starting point for biofilm growth [@yaoIdentificationProtein2003].
...
but are also necessary as a foundation to explain the growth of a biofilm.
...
The surface of some contains molecules (known as adhesins) that allow these bacteria to
attach to complementary receptors on the pellicle, so when the pellicle covers the tooth
surface it becomes a surface for bacterial attachment [@yaoIdentificationProtein2003].
The first bacteria
...
a limited ability to move if they have long tail-like structures known as
flagella, but most are brought to the surface by saliva.
...
pellicle proteins have a net negative charge [@songEffectsMaterialProperties2015],
causing eloctrostatic repulsive force, and attraction from van der Waals forces.
Depending on the distance from the bacteria to the surface, the bacteria may be
more or less likely to attach.
...
hydrodynamic forces, where hydrophobic components of the perllicle and cell surface
...
Attachment becomes stronger and colonisation becomes more solidified as
...
rather than directly to the tooth surface. This can occur on a large scale,
allowing the number and types of bacteria on the tooth surface to grow due to the ability
of different species to attach to one another (coaggregation)
[@jinSupragingivalCalculus2002; @marshDentalPlaque2006]. Coaggregation and coadhesion is an
important part of the growing oral biofilm. Most taxa don't have the necessary morphology
to attach directly to a substrate, however most oral taxa CAN coaggregate with
...
Firm attachment occurs at a shorter distance when bacteria are more directly
...
These are mainly *Streptococcus* spp. After direct attachment to the surface,
coadhesion allows bacteria of other
types to attach to the already attached bacteria. For example, *S. gordonii*
can attach to both the pellicle and *A. naeslundii* [@palmerCoaggregationInteractions2003].
There are many more examples of these bacteria-host and bacteria-bacteria
interactions that
...
early-colonising streptococci gradually decreases while
there is an increase of *Tannerella forsythia* *Actinomyces* spp. and *Fusobacterium nucleatum*
...
biofilms, which tend to be dominated by species with a lower oxygen tolerance
[@curtisRoleMicrobiota2020].
...
So far, the picture of biofilm formation is one of peaceful coexsistance,
...
The metabolic
by-products produced by some bacteria may be toxic for others, and allow them
to gain a competitive advantage. The aforementioned acid-production by some
bacteria can cause
...
*S. sanguinis* and *S. gordonii* can produce H_2_O2 that is toxic to *S. mutans*,
...
Competition between and within species will ultimately shape the wider
community. In addition to H~2~O~2~, oral streptococci can produce lactate
...
and 'killing' [@renduelesMechanismsCompetition2015].
...
of ions surpassess a critical threshold to overcome the balance between dissolution
and precipitation, salts will build up on the surface and within the biofilm.
...
rats [@theiladeGermfreeCalculus1964; @glasBiophysicalStudies1962]. It's unclear
...
[@flemmingBiofilmsEmergent2016; @wongCalciumPhosphate2002; @sissonsPHResponse1994]. In a
similar way, arginine can be broken down to ammonia and increase in pH.
Extended fluctuations
...
the heterogenous composition. Overall, plaque mineralisation is a complex interaction
between conditions in the local environment, availability
...
There are many choices to be made when growing a biofilm, such as the
composition of the initial oral microbial community, nutrient content
and availability,
and the makeup of the atmosphere [surrounding] the model.
As such, biofilm models can differ widely in their complexity and
ability to mimic conditions in a human mouth. 
A choice of model can be made based on the goal of the research ... , or
in some cases made for you based on (a lack of) available equipment and
financial constraints.
All models must have a defined [environment] containing the substratum and
nutrients. The substrata are the surfaces on which the biofilm is intended
to form and grow. For oral biofilm models the environment is the oral cavity
and the substrata are the teeth, tongue, mucosa, or whatever the model isthe biofilm
supposed to be mimicing.
The simplest models
...
More complicated models can involve increasingly sophisticated equipment to
...
The purpose of the incoulate is to initiate biofilm formation by allowing the
bacteria to adsorb to the substrata, ideally in the presence of a conditioning
film, such as saliva. For pure cultures and defined consortia, the saliva has
to be sterilised <!-- citations -->. For microcosms, the inoculate is often the
saliva itself, or dental plaque.
...
It's not always possible to use saliva as a medium for the duration of the
experiment, especially if the experiment lasts more than a few days.

  determines the
complexity of the biofilm microbiome
...
Finally, the artificial mouth model provides control over a large number of
